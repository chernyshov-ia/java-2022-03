...
public class MessageController {
    ...
    private static final long SPECIAL_ROOM = 1408;
    ...
    @SendTo({TOPIC_TEMPLATE + "{roomId}", TOPIC_TEMPLATE + SPECIAL_ROOM})
    public Message getMessage(@DestinationVariable String roomId, Message message) {
        ...
        if (String.valueOf(SPECIAL_ROOM).equals(roomId)) {
            throw new ChatException("not allowed here");
        }
    ...
    private Flux<Message> getMessagesByRoomId(long roomId) {
        var uri = SPECIAL_ROOM==roomId?"/msg-all":String.format("/msg/%s", roomId);
        logger.info("uri = {}, roomId = {}", uri, roomId);
